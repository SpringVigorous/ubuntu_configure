...
std::string get_xml_node_attribute_str(const tinyxml2::XMLElement* node, const std::string& attribute, xml_tree_encoding ec/* = xml_tree_encoding::unknown*/)
{
	std::string result = "";
	if (!node) return result;

	// 属性名一般不用中文，一般情况下只用英文和数字还有下划线组合，且不以数字开头，因此不用对属性按xml编码进行转换
	const char* res = node->Attribute(attribute.c_str());
	if (res)
	{
		...
		if (xml_tree_encoding::utf_8 == ec)
			result = string_tools::utf8_to_gbk(attr); // utf8
		else if (xml_tree_encoding::utf_8 == ec)
			result = attr; // 为MBCS编码
		else
			result = attr; // 其它编码暂不支持，当作MBCS处理
	}
...
bool get_xml_node_attribute_bool(const tinyxml2::XMLElement* node, const std::string& attribute)
{
	if (!node) return false;

	// 属性名一般不用中文，一般情况下只用英文和数字还有下划线组合，且不以数字开头，因此不用对属性按xml编码进行转换
	return node->BoolAttribute(attribute.c_str());
}

int get_xml_node_attribute_int(const tinyxml2::XMLElement* node, const std::string& attribute)
{
	if (!node) return 0;

	// 属性名一般不用中文，一般情况下只用英文和数字还有下划线组合，且不以数字开头，因此不用对属性按xml编码进行转换
	return node->IntAttribute(attribute.c_str());
}

unsigned int get_xml_node_attribute_uint(const tinyxml2::XMLElement* node, const std::string& attribute)
{
	if (!node) return 0;

	// 属性名一般不用中文，一般情况下只用英文和数字还有下划线组合，且不以数字开头，因此不用对属性按xml编码进行转换
	return node->UnsignedAttribute(attribute.c_str());
}

float get_xml_node_attribute_float(const tinyxml2::XMLElement* node, const std::string& attribute)
{
	if (!node) return 0.0f;

	// 属性名一般不用中文，一般情况下只用英文和数字还有下划线组合，且不以数字开头，因此不用对属性按xml编码进行转换
	return node->FloatAttribute(attribute.c_str());
}

double get_xml_node_attribute_double(const tinyxml2::XMLElement* node, const std::string& attribute)
{
	if (!node) return 0.0;

	// 属性名一般不用中文，一般情况下只用英文和数字还有下划线组合，且不以数字开头，因此不用对属性按xml编码进行转换
	return node->DoubleAttribute(attribute.c_str());
}

std::string get_xml_node_text(const tinyxml2::XMLElement* node, xml_tree_encoding ec /*= xml_tree_encoding::unknown*/)
{
	if (!node) return "";
	...
	std::string t_text;
	if (xml_tree_encoding::utf_8 == ec)
		t_text = string_tools::utf8_to_gbk(text);
