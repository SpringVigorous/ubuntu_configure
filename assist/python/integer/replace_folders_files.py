...
def main():
 ...
 #3[ -o F:/test_data/glm -d F:/test_data/ -r G:/练习集/C++/6.20/2.json ]
    ...
    parser.add_argument('-r', '--replace', type=str,  help='必选项：替换的特定名称对 1.列表或者2.json路径（仅替换参数或全参数）')
    ...
    if not replace_list:
        ...
        if not(full_json or replace_json) or full_json==replace_json:
            logger.error("json文件格式错误,示例如下：\n{full_json_str()}\n或者{args_json_str()}")
            return
