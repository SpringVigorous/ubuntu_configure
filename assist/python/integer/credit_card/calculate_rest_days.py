...
#当前周期 剩余天数
def cur_repay_days_rest(current_date, repayment_day):
    # 获取当前月份和年份
    ...
    try:
        # 计算账单日
        repayment_date = datetime.date(current_year, current_month, repayment_day)
...
#下一周期 剩余天数
def next_repay_days_rest(current_date, repayment_day, billing_day, is_billing_day_included=False):
    # 获取当前月份和年份
    current_month = current_date.month
    current_year = current_date.year
...
if __name__ == '__main__':
    ...
    days_until_repayment = next_repay_days_rest(current_date, repayment_day, billing_day, is_billing_day_included)
