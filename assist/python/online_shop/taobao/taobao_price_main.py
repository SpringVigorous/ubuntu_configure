...
result_dir=os.path.join(config.src_dir,"结果","淘宝")
...
def product_df():
    ...
    result["满减折扣"]=config.normal_cut_radio
...
if __name__=="__main__":
    ...
    result["定价"]=result.apply(lambda x: real_to_normal(x["成本价"],x["满减折扣"],x["优惠"]), axis=1)
