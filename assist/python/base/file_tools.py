...
def file_valid(file_path: str) -> bool | None:
    """
    传统 os 模块实现：判断文件是否为空
    返回值含义同上面的 pathlib 版本
    """
    detect_logger= logger_helper("检测文件状态",file_path)
    
    try:
        # 1. 判断路径是否存在
        if not os.path.exists(file_path):
            detect_logger.debug("路径不存在")
            return None
        
        # 2. 确保是文件（而非目录）
        if not os.path.isfile(file_path):
            detect_logger.debug("路径不是文件（是目录）")
            return None
        ...
        detect_logger.debug(f"未知错误",f"{e}")
        return None
...
async def __fetch_async(url ,session,max_retries=3,**kwargs):
    ...
    while retries < max_retries:
        try:
            ...
            async_logger.error("失败",  f"{retries+1} times,Request failed: {except_stack()}\n{e}",UpdateTimeType.ALL)
            retries += 1
...
async def _download_async(session:aiohttp.ClientSession,url,dest_path,lat_fun=None,covered=False,**kwargs):
    ...
    if lat_fun:
        ...
        try:
            ...
            async_logger.error("转码失败","忽略编码转换",update_time_type=UpdateTimeType.STAGE)
            # return False
        # async_logger.trace(f"pre-{count},latter-{len(content)}" )
    else:
        async_logger.trace("获取完成","无编码转换",update_time_type=UpdateTimeType.STAGE)
...
def download_sync(url,dest_path,lat_fun=None,covered=False,**kwargs):
    ...
    download_logger.debug("完成",None if lat_fun else "无编码转换", update_time_type=UpdateTimeType.ALL)
    return True
