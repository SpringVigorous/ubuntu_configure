...
from base import ThreadTask,get_param_from_url,find_last_value_by_col_val,ThreadPool,get_next_filepath,write_to_json_utf8_sig
...
class InteractImp():
    ...
    def handle_goods(self,url,itemId):
        goodsId=get_param_from_url(url,"id")
        logger=self.logger
        logger.update_target(f"收到第{self.msg_count}个消息:{url}",f"当前id:{goodsId}")
        logger.trace("开始",update_time_type=UpdateTimeType.STAGE)
        random_sleep(3,10,logger)
        if self.manager.has_id_val(self.manager.pic_df,item_id,goodsId):
        ...
        #写入
        self.cache_thread.submit(write_to_json_utf8_sig,get_next_filepath(main_dir,f"main_{goodsId}",".json"),main_pic_result)  
        self.cache_thread.submit(write_to_json_utf8_sig,get_next_filepath(desc_dir,f"desc_{goodsId}",".json"),desc_pic_result)  
...
class InteractProduct(ThreadTask):
    ...
    def _handle_data(self, data):
        url,itemId=data
        result=self.interact.handle_goods(url,itemId)
        return result
...
class HandlePics(ThreadTask):
    ...
    def _handle_detail_data(self,data):
        ...
        product_df=pd.concat([main_df,desc_df])
...
class DownloadPics(ThreadTask):
    ...
    @exception_decorator()
    def _handle_data(self, data):
        return self._download_by_df(data)
    def _download_by_df(self,data)->list[str]:
        df:pd.DataFrame=data[1]
        if df.empty:
            return
        
        self.cache_thread.reset()
        logger=self.logger
...
class OcrPics(ThreadTask):
    ...
    @exception_decorator()
    def _handle_data(self,data):
        return self._ocr_by_lst(data)
        
    def _ocr_by_lst(self,lst:list[str]):
        self.cache_thread.reset()
        logger=self.logger
        
        ocr_lst:list[dict]=[]
        def _ocr_pics(org_path,ocr_path):
            logger.update_target(detail=f"{org_path}->{ocr_path}")
            texts=None
            try:
                ...
                if texts:
                    ocr_lst.append({name_id:Path(ocr_path).stem,ocr_text_id:";".join(texts)})
        for pic_path in lst:
            ocr_path= os.path.join(ocr_pic_dir, Path(pic_path).name)
            self.cache_thread.submit(_ocr_pics,pic_path,ocr_path)
        self.cache_thread.shutdown()
        if ocr_lst:
            ocr_df=pd.DataFrame(ocr_lst)
